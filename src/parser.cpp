...
void Parser::parse()
{
    ...
    while (true)
    {
        if (lexer.isTokenIntDC())
            ParseVariableDeclarationStatement(true)->codegen();
        else if (lexer.isTokenInt() || lexer.isTokenDouble() || lexer.isTokenBoolean() || lexer.isTokenStringType())
            ParseVariableDeclarationStatement(false)->codegen();
...
unique_ptr<Statement> Parser::ParseVariableDeclarationStatement( bool flag)
{
...
unique_ptr<Statement> Parser::ParseForStatement()
{
    ...
    {
        auto rtl = ParseVariableDeclarationStatement(false);
