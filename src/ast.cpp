...
void BinaryExpressionTwo::VarDecCodeGen(GlobalVariable *gVar, ::Type *vt)
{
    gVar->setInitializer(vt->getDefaultConstant());
    cg->builder->SetInsertPoint(cg->getCOPBB());
    llvm::Value *v = codeGen();
...
llvm::Value *BinaryExpressionTwo::codeGen()
{
    llvm::Value *lhs = LVAL->codeGen();
    llvm::Value *mhs = MVAL->codeGen();
    llvm::Value *rhs = RVAL->codeGen();
    
    auto expr1 = op->codeGen(lhs,mhs);
    auto expr2 = op2->codeGen(mhs,rhs);

    return cg->builder->CreateAnd(expr1, expr2, "andop");;
}
